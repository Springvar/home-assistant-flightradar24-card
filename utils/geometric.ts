import type { Position } from '../types/flight';
...
export function calculateNewPosition(
    ...
    const newLatRad = Math.asin(
        Math.sin(latRad) * Math.cos(distanceRad) + Math.cos(latRad) * Math.sin(distanceRad) * Math.cos(bearingRad)
    );
    const newLonRad =
        lonRad +
        Math.atan2(
...
export function calculateClosestPassingPoint(
    refLat: number,
    refLon: number,
    flightLat: number,
    flightLon: number,
...
export function areHeadingsAligned(
    direction_to_tracker: number,
    heading: number,
    margin: number = 60
): boolean {
